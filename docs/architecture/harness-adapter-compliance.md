...
| Grok Build | Adapter-backed | Grok plugin and marketplace manifests; shared skills; commands; hooks after `--trust`; root `.mcp.json` after trust | Claude userConfig hook profiles are ignored; use ECC_HOOK_PROFILE; ECC agent tool allowlists use Claude names and are not mapped in Grok agent frontmatter; No plugin rules/ surface; no ecc install --target grok | `grok plugin marketplace add affaan-m/ECC && grok plugin install ecc --trust && grok plugin enable ecc`; `grok plugin install /path/to/ECC --trust && grok plugin enable ecc` | `grok plugin validate .`; `node tests/plugin-manifest.test.js`; `node tests/lib/resolve-ecc-root.test.js` | Grok rejects Claude marketplace source "./"; keep a url catalog in .grok-plugin/marketplace.json; Trusted installs attach chrome-devtools from root .mcp.json; the full skill catalog is large |
